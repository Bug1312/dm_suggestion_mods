...
public class PermissionScreen extends Screen {
	...
	private static final ITextComponent NONE_FOUND = (new TranslationTextComponent("gui.socialInteractions.search_empty")).withStyle(TextFormatting.GRAY);

	private TextFieldWidget searchBox;
	private String lastSearch = "";

	private FilterPermissionList tardisCallList;
	...
	public void tick() {
		...
		if (mustUpdate) {
			PermissionScreen.mustUpdate = false;
			tardisCallList.updateTardisList(height);
		}
	}
	...
	@Override
	protected void init() {
		ModMain.NETWORK.sendToServer(new PacketRequestPermissions());

		this.minecraft.keyboardHandler.setSendRepeatsToGui(true);
		if (this.initialized) this.tardisCallList.updateSize(this.width, this.height, 88, this.listEnd());
		else this.tardisCallList = new FilterPermissionList(this, Minecraft.getInstance(), this.width, this.height, 88, this.listEnd(), 36);

		String s = this.searchBox != null ? this.searchBox.getValue() : "";
		this.searchBox = new TextFieldWidget(this.font, this.marginX() + 28, 78, 196, 16, SEARCH_PLACEHOLDER) {
			protected IFormattableTextComponent createNarrationMessage() {
				return !PermissionScreen.this.searchBox.getValue().isEmpty() && PermissionScreen.this.tardisCallList.isEmpty() ? super.createNarrationMessage().append(", ").append(PermissionScreen.NONE_FOUND) : super.createNarrationMessage();
			}
		};
	...
	public void render(MatrixStack stack, int mouseX, int mouseY, float frameTick) {
		...
		if (!this.tardisCallList.isEmpty()) this.tardisCallList.render(stack, mouseX, mouseY, frameTick);
		else if (!this.searchBox.getValue().isEmpty()) drawCenteredString(stack, this.minecraft.font, NONE_FOUND, this.width / 2, (78 + this.listEnd()) / 2, -1);

		if (!this.searchBox.isFocused() && this.searchBox.getValue().isEmpty()) drawString(stack, this.minecraft.font, SEARCH_PLACEHOLDER, this.searchBox.x, this.searchBox.y, -1);
		else this.searchBox.render(stack, mouseX, mouseY, frameTick);
	...
	private void checkSearchStringUpdate(String search) {
		...
		if (!search.equals(this.lastSearch)) {
			...
			tardisCallList.updateTardisList(tardisCallList.getScrollAmount());
		}
	}
